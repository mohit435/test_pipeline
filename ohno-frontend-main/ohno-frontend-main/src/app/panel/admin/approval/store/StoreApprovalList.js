...
import ViewPurchaseIndent from '@/app/panel/store/approvals/ViewPurchaseIndent';
import ViewPO from '@/app/panel/store/approvals/ViewPO';
...
const StoreApprovalList = () => {
    ...
    const [piAndPoData, setPiAndPoData] = useState({});
    const [modelState, setModelStates] = useState({
        viewPI: false,
        viewPO: false,
    });
    ...
    // get approval list on initial load
    const getApproval = async (controller) => {
        try {
            setLoader(true);
            const serverResponse = await communication.fetchStoreApprovalData(
                "",
                controller
            );
            if (serverResponse?.data?.status === "SUCCESS") {
                setApprovalData(serverResponse?.data?.data);
                setLoader(false);
    ...
    // get store approval data by id
    const getApprovalDataById = async (id, identifier) => {
        try {
            const serverResponse = await communication.fetchStoreApprovalDataById(
                id,
                identifier
            );
            if (serverResponse?.data?.status === "SUCCESS") {
                setPiAndPoData(serverResponse?.data?.data);
            } else if (serverResponse?.data?.status === "JWT_INVALID") {
                router.push("/");
            } else {
                setPiAndPoData({});
    ...
    return (
        <>
            ...
            }
            {
                ...
                modelState?.viewPI && <ViewPurchaseIndent data={piAndPoData} setModelStates={setModelStates} isApproved={isApproved} setIsApproved={setIsApproved} />
            }
            {
                // Purchase Order approval 
                modelState?.viewPO && <ViewPO data={piAndPoData} setModelStates={setModelStates} isApproved={isApproved} setIsApproved={setIsApproved} />
            }
                    ...
                    <>
                        {
                            approvalData?.map((data, index) => {
                                return (
                                    <div className="col-lg-6 mb-4  card-anim" key={index}>
                                        <div className={`approval_box`} >
                                            <div className={`approval_card`}>
                                                <div className="approval_time">
                                                    <div className="time_card py-4">
                                                        <p className='small'>{getDateDescription(data?.createdAt)}</p>
                                                        <h6>
                                                ...
                                                <div className="approval_action">
                                                    <div title="view"
                                                        onClick={() => {
                                                            data?.identifier === "PO" &&
                                                                setModelStates((prev) => ({ ...prev, viewPO: true }))
                                                            getApprovalDataById(data?._id, data?.identifier)
                                                            data?.identifier === "PI" &&
                                                                setModelStates((prev) => ({ ...prev, viewPI: true }))
                                                            getApprovalDataById(data?._id, data?.identifier)
                                                        }}
                                                    >
