...
import HrmApprovalDetail from './HrmApprovalDetails';
...
const HrmApprovalList = () => {
    const router = useRouter();
    const [loader, setLoader] = useState(false);
    const [promotionList, setPromotionList] = useState([]);
    const [promotionDetail, setPromotionDetail] = useState({});
    const [isPromotionApproved, setIsPromotionApproved] = useState(false);
    const [toggleApprovalView, settoggleApprovalView] = useState(false);

    // get all promotion list on initial load
    const getPromotionList = async (controller) => {
        try {
            setLoader(true);
            const serverResponse = await communication.fetchPromotionList(controller);
            if (serverResponse?.data?.status === "SUCCESS") {
                setPromotionList(serverResponse?.data?.result);
                setLoader(false);
    ...
    const getPromotionById = async (id) => {
        try {
            const serverResponse = await communication.fetchPromotionById(id);
            if (serverResponse?.data?.status === "SUCCESS") {
                setPromotionDetail(serverResponse?.data?.result);
            } else if (serverResponse?.data?.status === "JWT_INVALID") {
                router.push("/");
                toast.info(serverResponse?.data?.message)
            } else {
    ...
    return (
        <>
            ...
            {
                toggleApprovalView &&
                <HrmApprovalDetail data={promotionDetail} settoggleApprovalView={settoggleApprovalView} setIsPromotionApproved={setIsPromotionApproved} />
            }
            {/* promotion list  */}
...
export default HrmApprovalList